...
export function AccountRow({ productId, account }: AccountRowProps) {
  ...
  const badge = !isAvailable && account.availableAt ? (
    <span className="text-xs font-mono font-semibold text-amber-600 bg-amber-50 dark:bg-amber-500/10 dark:text-amber-400 px-2.5 py-0.5 rounded-md select-none border border-amber-200/50 dark:border-amber-900/30 whitespace-nowrap">
    ...
    <span className="text-xs font-mono font-medium text-violet-600 dark:text-violet-400 bg-violet-50/80 dark:bg-violet-500/10 px-2.5 py-0.5 rounded-md select-none flex items-center gap-1.5 border border-violet-100/40 dark:border-violet-900/15 whitespace-nowrap">
      <RefreshCcw className="w-2.5 h-2.5 animate-spin text-violet-500 flex-shrink-0" style={{ animationDuration: '6s' }} />
      <span className="hidden sm:inline text-[9px] text-violet-400 dark:text-violet-500 font-bold uppercase tracking-wider">Reset in:</span>
  ...
  return (
    <div className="flex items-center px-3 py-2.5 rounded-xl transition-colors group relative bg-transparent hover:bg-gray-50/60 dark:hover:bg-white/5 justify-between">
        ...
        <div className="grid grid-cols-[1fr_auto] sm:grid-cols-[200px_auto] md:grid-cols-[240px_auto] lg:grid-cols-[280px_auto] items-center gap-4 min-w-0 flex-1">
          <div className="min-w-0 flex flex-col gap-0.5">
            <EditableText
              ...
              trigger="doubleClick"
            />
            {/* Badge on mobile: sits below the name to avoid horizontal overflow */}
            {badge && <div className="sm:hidden pl-1">{badge}</div>}
          </div>
      ...
      {/* Badge on sm+: right-aligned on the same row */}
      {badge && (
